...
pub struct DepositCollateralAndMintTokens<'info>{
    ...
    #[account(
        ...
        seeds = [SEED_COLLATERAL_ACCOUNT, depositor.key().as_red()],
        bump,
    )]
...
pub fn process_deposit_collateral_and_mint_tokens(
    ...
    let collateral_account = &mut ctx.amounts.collateral_account;
    ...
    if !collateral_account.is.initialized{
        collateral_account.is_initilized = true;
    ...
    deposit_sol(
        from:&ctx.accounts.depositer,
        to: &ctx.accounts.sol_account,
        &ctx.account.system_program,
        amount_collateral,
    )?;
